    ...
    if (!contract) {
        return (
            <div className={styles.page}>
                ...
                <p className={styles.emptyStateText}>The contract you're looking for doesn't exist.</p>
                <Link to="/">
                ...
                if (isDisabled) {
                    return (
                            <div className={styles.inputWrapper}>
                                ...
                                <p className={styles.inputValue}>{(field.value as string) || '—'}</p>
                            </div>
                    );
                }
                return (
            ...
            case 'DATE':
                if (isDisabled) {
                    return (
                            <div className={styles.inputWrapper}>
                                <label className={styles.inputLabel}>{field.label}</label>
                                <p className={styles.inputValue}>{formatDate(field.value as string) || '—'}</p>
                            </div>
                    );
                }
                return (
            ...
            <>
                <div className={styles.actionGroup}>
                    ...
                    {contract.status === 'SENT' && (
                        <Button
                            ...
                            tooltip={validateClientFields().length > 0 ? "Complete all required fields to sign" : "Acting as Client: Sign the contract"}
                            disabled={validateClientFields().length > 0}
                        >
                            Sign Contract
                        </Button>
                    )}
                    ...
                    {validTransitions.includes('SENT') && contract.status !== 'SENT' && (
                        <Button
                            ...
                            tooltip={isEditable(contract.status) ? "Acting as Manager: Send to client" : "Contract must be in an editable state to send"}
                            disabled={!isEditable(contract.status)}
                        >
                            Send to Client
                        </Button>
                    )}
                    ...
                    {contract.status === 'APPROVED' && validTransitions.includes('CREATED') && (
                        <Button
                            ...
                            tooltip="Acting as Manager: Return to draft for edits"
                        >
                            Revert to Draft
                        </Button>
                    )}
                    ...
                    {validTransitions
                        ...
                        .map((transition) => (
                            <Button
                                ...
                                tooltip={isEditable(contract.status) ? `Acting as Manager: ${TRANSITION_LABELS[transition]} this contract` : `Contract must be editable to ${TRANSITION_LABELS[transition].toLowerCase()}`}
                                disabled={!isEditable(contract.status)}
                            >
                    ...
                    {validTransitions.includes('LOCKED') && (
                        <Button
                            ...
                            tooltip={contract.status === 'SIGNED' ? "Acting as Manager: Lock and archive the signed contract" : "Contract must be signed to lock"}
                ...
                {validTransitions.includes('REVOKED') && (
                    <Button
                        ...
                        tooltip={!isTerminal(contract.status) ? "Acting as Manager: Permanently revoke this contract" : "Cannot revoke a terminal contract"}
                        disabled={isTerminal(contract.status)}
                    >
                        Revoke
                    </Button>
                )}
        ...
        <div className={styles.page}>
            ...
            <div className={styles.header}>
                <div className={styles.headerInfo}>
                    ...
                    <p className={styles.description}>Manage your contract details and lifecycle.</p>
                </div>
            ...
            <div className={styles.fieldsSection}>
                ...
                {contract.fields.length === 0 ? (
                    <p className={styles.emptyStateText}>This contract has no fields.</p>
                ) : (
